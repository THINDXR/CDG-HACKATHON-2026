...
window.Store = (function () {
   ...
   *
   * ระดับบนสุดใช้คำว่า "ระวังให้มาก" ไม่ใช่ "อันตรายมาก" เพราะคะแนนนี้รวม
   * ผลจากโมเดลพยากรณ์เข้ามาด้วย ซึ่งเป็นการคาดการณ์ ไม่ใช่การยืนยันว่าจะเกิด
   * คำว่า "อันตราย" ฟังเหมือนรับรองว่ามีอันตรายอยู่จริงตรงนั้นตอนนี้
   */
  const RISK_LEVELS = [
    {
      min: 70, key: 'critical', label: 'ระวังให้มาก', color: '#ff3b30',
      advice: 'ความเสี่ยงสูงกว่าปกติมาก เลี่ยงเส้นทางนี้ได้ก็ควรเลี่ยง',
    },
    {
  ...
  function riskAssessment(origin) {
    ...
    /*
     * ตัดปลายทั้งสองข้างไว้ที่ 5-95
     *
     * คะแนนนี้รวมผลจากโมเดลพยากรณ์ซึ่งเป็นการคาดการณ์ ไม่ใช่ความจริงที่วัดได้
     * "อันตราย 100%" จึงเป็นการรับรองสิ่งที่รับรองไม่ได้ ส่วน "ปลอดภัย 100%"
     * อันตรายกว่าอีก เพราะทำให้ผู้ใช้เลิกระวังทั้งที่ระบบไม่มีทางรู้ได้ขนาดนั้น
     * (เกณฑ์เดียวกับที่ใช้กับความน่าจะเป็นของโมเดลใน ai-model.js)
     */
    const raw01 = Math.min(100, Math.max(density, immediate));
    const score = Math.round(Math.min(95, Math.max(5, raw01)));
